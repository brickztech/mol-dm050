...
from collections.abc import Mapping
from dataclasses import dataclass, replace
from typing import Callable,Iterator,Any,Annotated,TypeAlias,Tuple,final,cast
import json
...
class ToolResultEntry(Entry):
    ...
    result  : Annotated[str,"The result in string format. This can be a simple string value like '20250801' or the string representation of a JSON object"]
...
class LLM(ABC):
    ...
    def streamanswer(self, query: Query,
                     temperature: float | None = None, 
                     model: str | None = None,
                     toolfunction: ToolFunction | None = None, 
                     cycle_limit: int | None = None, 
...
class ManualLLM(LLM):
    ...
    def _step(self, query: Query, stream: bool) -> Reply:
        while True:
            ...
            if responsetype == '1': # TextReply
                ...
                toolname: str = (input("Enter the name of the first tool to be called or '/' to return to the reply type choice: ")
                                 .strip())
                if toolname == "/":
                    continue
                argname: str = ""
                while True:
                    ...
                    argname = (input("Enter name of first argument, '.' to finish the list of arguments or '/' to return to the reply type choice: ")
                               .strip())
                    while True:
                        ...
                        argname = (input("Enter name of next argument, '.' to finish the list of arguments or '/' to return to the reply type choice: ")
                                   .strip())
                    if argname == '/':
                        break
                    calls.append(SingleToolCall(str(ManualLLM._next_call_id),toolname,args))
                    ManualLLM._next_call_id += 1
                    toolname = (input("Enter the name of the next tool to be called, '.' to finish or '/' to return to the reply type choice: ")
                                .strip())
    ...
    def answer(self,query: Query,
        ...
        print(f"ManualLLM.answer() invoked with temperature = {temperature}, model = {model}, toolfunction = {toolfunction.__qualname__ if toolfunction else None}, cycle_limit = {cycle_limit} and **kwargs = {kwargs}")
        iterations: int = 0
        while True:
            reply: NonStreamingReply = self.step(query,temperature=temperature,model=model,**kwargs)
            match reply:
    ...
    def streamanswer(self,query: Query,
        ...
        print(f"ManualLLM.stramanswer() invoked with temperature = {temperature}, model = {model}, toolfunction = {toolfunction.__qualname__ if toolfunction else None}, cycle_limit = {cycle_limit} and **kwargs = {kwargs}")
        iterations: int = 0
        while True:
            reply: StreamingReply = self.streamstep(query,temperature=temperature,model=model,**kwargs)
            match reply:
...
default_model={default_model},default_temperature={default_temperature}"
    ...
    def _resolve_named_parameters(self,
                                  ...
                                  model: str | None,
                                  toolfunction: ToolFunction | None,
    ...
    def _tokenstream_of(self, chunkstream: Iterator[Any], firstmessage: Any) -> Iterator[str]:
        ...
        while True:
            try:
                if ((delta_end := getattr(nextmessage,"delta",None)) is not None 
                ...
                raise LLMBlockzException("Raw stream came to an end in OpenAILLM._tokenstream_of without first seeing finish_reason = 'stop'")
    ...
    def _calls_of(self,query: Query, chunkstream: Iterator[Any], firstmessage: Any) -> list[SingleToolCall]:
        ...
        while True:
            try:
                if ((delta_end := getattr(nextmessage,"delta",None)) is not None 
                    and (toolcalls_end := getattr(delta_end,"tool_calls",None)) is not None
                   ):
                ...
                raise LLMBlockzException("Raw stream came to an end in OpenAILLM._toolcalls_of without first seeing finish_reason = 'tool_calls'")
        for _ in chunkstream:
            pass
        calls: list[SingleToolCall] = []
        for call in collection.values():
            if ("call_id" not in call or
                "name" not in call or
                "argl" not in call):
    ...
    def streamstep(self,query: Query, temperature: float | None = None, model: str | None = None, **kwargs: Any) -> StreamingReply:
        temperature,model,_toolfunction,_cycle_limit = self._resolve_named_parameters(temperature,model,None,None)
        logger.debug(f"OpenAILikeLLM.streamstep() called with temperature={temperature}, model={model}, extra arguments={kwargs}\nQuery={query}")
