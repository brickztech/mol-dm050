...
"""Recursive Dicts from string to either string or another recursive dictionary. Common type for LLM communications"""
...
def as_dictstrstr(v: RecStrDictValue) -> dict[str, str]:
...
class SingleToolCall:
    ...
    paramvalues: Annotated[dict[str, str], "The parameter values in a string-to-string dictionary."]
...
def serialize_SingleToolCall(stcall: SingleToolCall) -> RecStrDict:
    match stcall:
        case SingleToolCall(call_id=call_id, name=name, paramvalues=paramvalues):
            ...
            typed_paramvalues: RecStrDict = paramvalues  # type: ignore
            return {'call_id': call_id, 'name': name, 'paramvalues': typed_paramvalues}
        case _:
...
def _evaluate_singletoolcall(toolfunction: Callable[[str, dict[str, str]], str], singletoolcall: SingleToolCall) -> ToolResultEntry:
...
def serialized_History(history: History) -> list[RecStrDict]:
    ...
    for entry in history:
        match entry:
            ...
            case RawEntry(content=content):
                retval.append({"raw": f"{json.dumps(content)}"})
            case _:
...
def deserialized_History(dictlist: list[RecStrDict]) -> History:
    ...
    for d in dictlist:
        if len(d) != 1:
            ...
            v = as_string(d["calls"])
            retval.append(ToolCallsEntry([deserialize_SingleToolCall(stc) for stc in json.loads(v)]))
        elif "tool" in d:
...
def print_Parameter(indent: str, parameter: Parameter) -> None:
    ...
    match parameter:
        case Parameter(name=name, typ=typ, description=description, must=must):
            print(indent, f"{name} : {typ} -- {description}", sep="")
        case _:
...
def _valid_call_against_descr(paramlist: list[Parameter], paramvaluedict: dict[str, str]) -> bool:
...
ToolDict = dict[str, ToolDescription]
...
class Query:
    ...
    systeminstr: Annotated[str, "The system instruction for this query"]
    tools: Annotated[ToolDict, "Tool dictionary to be transferred to the LLM - this is what it can choose from."]
    ...
    def _copy(self) -> 'Query':
        """Shallow copy of a Query, with the exception of history, which is itself shallow copied instead of copied by reference"""
        retval: Query = Query.empty()
        retval.tools = self.tools
        retval.systeminstr = self.systeminstr
        retval.history = self.history[:]
        return retval

    def with_user(self, content: str) -> 'Query':
        """APPENDS an user entry to the history"""
        retval: Query = self._copy()
        retval.history.append(UserEntry(content))
        return retval

    def with_assistant(self, content: str) -> 'Query':
        """APPENDS an user entry to the history"""
        retval: Query = self._copy()
        retval.history.append(AssistantEntry(content))
        return retval

    def with_toolcalls(self, calls: list[SingleToolCall]) -> 'Query':
        """APPENDS a toolcalls message to the history"""
        retval: Query = self._copy()
        retval.history.append(ToolCallsEntry(calls))
        return retval

    def with_toolreply(self, call_id: str, name: str, result: str) -> 'Query':
        """APPENDS a toolreply message to the history"""
        retval: Query = self._copy()
        retval.history.append(ToolResultEntry(call_id, name, result))
        return retval

    def with_raw(self, content: RecStrDict) -> 'Query':
        """APPENDS a raw message to the history."""
        retval: Query = self._copy()
        retval.history.append(RawEntry(content))
        return retval

    def with_history(self, history: History) -> 'Query':
        """REPLACES self.history with the parameter."""
        retval: Query = self._copy()
    ...
    def last_message(self) -> str:
        ...
        lastentry: Entry = self.history[-1]
        match lastentry:
            case UserEntry(content=content):
                return content
            case _:
    ...
    def __repr__(self) -> str:
        return f"""Query(history={self.history},
systeminstr={self.systeminstr},
tools={self.tools})"""
...
def print_Query(indent: str, query: Query) -> None:
    ...
    match query:
        case Query(history=history, systeminstr=systeminstr, tools=tools):
            ...
            print_History(indent + incr, query.history)
        case _:
...
ToolFunction: TypeAlias = Callable[[str, dict[str, str]], str]
...
class Reply:
    ...
    originalquery: Annotated[Query, "The query that led to this reply."]
...
class RootReply(NonStreamingReply):
    ...
    def newquery(self, userinput: str, *historyfilters: Callable[[History], History]) -> Query:
        ...
        """
        newhistory: History = self.originalquery.history
        for historyfilter in historyfilters:
            newhistory = historyfilter(newhistory)
        return self.originalquery.with_history(newhistory).with_user(userinput)


def trim_toolexchanges(history: History) -> History:
    """Removes the entries related to tool invocation and tool call results from the end of history"""
    retval = history[:]
    while retval and is_tool_Entry(retval[-1]):
        retval.pop()
    return retval
...
class TextReply(NonStreamingReply):
    ...
    text: Annotated[str, "The text received from the LLM."]
    ...
    def newquery(self, userinput: str, *historyfilters: Callable[[History], History]) -> Query:
        ...
        """
        if historyfilters == ():
            historyfilters = (trim_toolexchanges,)
        logger.debug(
        ...
        for historyfilter in historyfilters:
            newhistory = historyfilter(newhistory)
        return self.originalquery.with_history(newhistory).with_assistant(self.text).with_user(userinput)
...
class ToolCallsReply(NonStreamingReply):
    ...
    def evaluate(self, toolfunction: ToolFunction) -> Query:
        ...
        toolcallsentry: ToolCallsEntry = ToolCallsEntry(self.calls)
        toolresultentries: list[Entry] = [_evaluate_singletoolcall(toolfunction, singletoolcall) for singletoolcall in self.calls]
        return self.originalquery.with_history(self.originalquery.history + [toolcallsentry] + toolresultentries)
...
def print_NonStreamingReply(indent: str, nonstreamingreply: NonStreamingReply) -> None:
    ...
    match nonstreamingreply:
        case TextReply(originalquery=originalquery, text=text):
            print(indent, "<TextReply>", sep="")
            print_Query(indent + incr, originalquery)
            print(indent + incr, f"Text: {text}", sep="")
        case ToolCallsReply(originalquery=originalquery, calls=calls):
...
class StreamingTextReply(StreamingReply):
    ...
    def __init__(self, originalquery: Query, rootstream: Iterator[str]) -> None:
        super().__init__(originalquery)
...
class StreamingToolCallsReply(StreamingReply):
    ...
    def __init__(self, originalquery: Query, callsbuilder: Callable[[], list[SingleToolCall]]) -> None:
        super().__init__(originalquery)
    ...
    def evaluate(self, toolfunction: ToolFunction) -> Query:
        ...
        toolcallsentry: ToolCallsEntry = ToolCallsEntry(self._calls)
        toolresultentries: list[Entry] = [_evaluate_singletoolcall(toolfunction, singletoolcall) for singletoolcall in self._calls]
        logger.debug(f"calls={self._calls}")
        return self.originalquery.with_history(self.originalquery.history + [toolcallsentry] + toolresultentries)


def manual_toolfunction(name: str, params: dict[str, str]) -> str:
...
def sentinel_toolfunction(name: str, params: dict[str, str]) -> str:
...
class OpenAILikeLLM(LLM):
    ...
    def __init__(self, client: Any, default_model: str, default_temperature: float = 0.7) -> None:
        super().__init__()
        logger.debug(f"OpenAILikeLLM constructor called with ")
...
default_model={default_model},default_temperature={default_temperature}"
    ...
    def _format_param_properties(self, parameter: Parameter) -> RecStrDict:
        if parameter.description == "":
            ...
            return {"type": parameter.typ, "description": parameter.description}
    ...
    def _format_singletoolcall(self, singletoolcall: SingleToolCall) -> RecStrDict:
        match singletoolcall:
            case SingleToolCall(call_id=call_id, name=name, paramvalues=paramvalues):
                return {"id": call_id, "type": "function", "function": {"name": name, "arguments": json.dumps(paramvalues)}}
            case _:
    ...
    def step(self, query: Query, temperature: float | None = None, model: str | None = None, **kwargs: Any) -> NonStreamingReply:
        ...
        if isinstance(essence, str):
            ...
            raise TypeError("Return type from LLM._step is neither string nor list (of SingleToolCalls)")
    ...
    def streamstep(self, query: Query, temperature: float | None = None, model: str | None = None, **kwargs: Any) -> StreamingReply:
        ...
        if self._can_stream:
            ...
            if isinstance(essence, str):
                ...
                raise LLMBlockzException("OpenAILikeLLM._step() returned neither string nor calls list.")
